...
__forceinline float Fresnel(const Vector& wo, Vector& normal, float n, Vector* reflect, Vector* refract)
{
	...
	*reflect =2 * cosa * normal - wo;//��������

	if (n != 0)
	{
		auto cosi = cosa;                           //cos(�����/�����)
		auto cost = 1 - (1 - cosi * cosi) / (n * n);//cos(�����)
		if (cost > 0) //��ȫ����
		{
			cost = sqrt(cost);
...
Color Trace(const Ray& ray, const Scene& scene, int depth)
{
	...
	auto pos = hittest.Distance * ray.Direction + ray.Position; //����λ��
	auto normal = hittest.Normal;                               //���㷨����
	auto cosa = normal * d;                                     //-Cos(�����)
