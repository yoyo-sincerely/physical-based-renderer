...
static void ComputeContext(const float* view, const float* projection, float* matrix, mode mod)
{
	gContext.mMode = mod;
...
static void ComputeColors(ImU32* colors, int type, operation op)
{
		...
		switch(op)
		{
			case translate:
				...
				for(int i = 0; i < 3; i++)
				{
					...
					colors[i + 4] =
						(type == (int)(MOVE_XY + i)) ? selectionColor : planeColor[colorPlaneIndex];
...
static void DrawRotationGizmo(int type)
{
	...
	for(int axis = 0; axis < 3; axis++)
	{
		...
		float angleStart =
			atan2f(cameraToModelNormalized[(4 - axis) % 3], cameraToModelNormalized[(3 - axis) % 3]) +
			ZPI * 0.5f;
	...
	for(unsigned int i = 0; i < 3; i++)
	{
		...
		if(belowAxisLimit)
		{
			...
			ImVec2 worldDirSSpace =
				worldToPos((dirPlaneX * scaleDisplay[i]) * gContext.mScreenFactor, gContext.mMVP);
...
static int GetScaleType()
{
	...
	for(unsigned int i = 0; i < 3 && type == NONE; i++)
	{
		...
		const float dx =
			dirPlaneX.Dot3((posOnPlan - gContext.mModel.v.position) * (1.f / gContext.mScreenFactor));
		const float dy =
			dirPlaneY.Dot3((posOnPlan - gContext.mModel.v.position) * (1.f / gContext.mScreenFactor));
		...
		if(CanActivate() && type != NONE)
		{
			...
			if(applyRotationLocaly)
			{
				gContext.mTranslationPlan =
					BuildPlan(gContext.mModel.v.position, rotatePlanNormal[type - ROTATE_X]);
			}
			else
			{
				gContext.mTranslationPlan =
...
void DecomposeMatrixToComponents(const float* matrix, float* translation, float* rotation, float* scale)
{
	...
	rotation[1] =
		RAD2DEG * atan2f(-mat.m[0][2], sqrtf(mat.m[1][2] * mat.m[1][2] + mat.m[2][2] * mat.m[2][2]));
